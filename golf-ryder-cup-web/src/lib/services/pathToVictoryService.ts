...
export function getQuickSummary(pathToVictory: PathToVictory): {
  ...
  } else if (teamA.pointsNeeded > remainingMatches) {
    // Cannot clinch outright - need opponent to drop points
    teamASummary = `${teamA.name} must win all ${remainingMatches} and needs help`;
  } else {
  ...
  } else if (teamB.pointsNeeded > remainingMatches) {
    // Cannot clinch outright - need opponent to drop points
    teamBSummary = `${teamB.name} must win all ${remainingMatches} and needs help`;
  } else {
