...
export class GolfTripDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(4).stores({
      ...
      tripStats:
        'id, tripId, playerId, sessionId, statType, [tripId+playerId], [tripId+statType], [playerId+statType]',
    ...
    this.version(7).stores({
      ...
      chatMessages:
        'id, tripId, threadId, authorId, timestamp, [tripId+timestamp], [threadId+timestamp]',
      ...
      photos:
        'id, tripId, albumId, uploaderId, uploadedAt, [tripId+uploadedAt], [albumId+uploadedAt]',
      ...
      headToHeadRecords:
        'id, tripId, player1Id, player2Id, [tripId+player1Id], [player1Id+player2Id]',
    ...
    this.version(8).stores({
      ...
      courseSyncQueue:
        '++queueId, courseProfileId, status, retryCount, createdAt, [status+retryCount]',
    });
...
export async function getSessionWithMatches(sessionId: string) {
  ...
  const matches = await db.matches.where('sessionId').equals(sessionId).sortBy('matchOrder');
...
export async function getMatchWithResults(matchId: string) {
  ...
  const holeResults = await db.holeResults.where('matchId').equals(matchId).sortBy('holeNumber');
...
export async function getTeamWithPlayers(teamId: string) {
  ...
  const members = await db.teamMembers.where('teamId').equals(teamId).sortBy('sortOrder');
...
export async function getScoringEventsForMatch(matchId: string) {
  return db.scoringEvents.where('matchId').equals(matchId).sortBy('timestamp');
}
...
export async function getUnsyncedEvents() {
  return db.scoringEvents.where('synced').equals(0).toArray();
}
