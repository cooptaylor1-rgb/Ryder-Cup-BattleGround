...
export interface SyncResult {
  ...
  errors: string[];
  timestamp: Date;
  /** Trip ID when joining a trip by share code */
  tripId?: string;
}
...
function convertKeysToSnakeCase<T extends Record<string, unknown>>(
  obj: T
...
class SyncService {
  ...
  async joinTripByShareCode(shareCode: string): Promise<SyncResult> {
    ...
    try {
      ...
      const trip = convertKeysToCamelCase(
      ...
      // Fetch teams
      const { data: teamsData } = await supabase.from('teams').select('*').eq('trip_id', trip.id);

      if (teamsData) {
        for (const teamData of teamsData) {
          const team = convertKeysToCamelCase(
      ...
      if (sessionsData) {
        for (const sessionData of sessionsData) {
          const session = convertKeysToCamelCase(
            sessionData as unknown as Record<string, unknown>
          ) as unknown as RyderCupSession;
        ...
        if (matchesData) {
          for (const matchData of matchesData) {
            const match = convertKeysToCamelCase(
          ...
          if (holeResultsData) {
            for (const hrData of holeResultsData) {
              const holeResult = convertKeysToCamelCase(
                hrData as unknown as Record<string, unknown>
      ...
      if (teamsData) {
        ...
        if (teamMembersData) {
          ...
          for (const tmData of teamMembersData) {
            const tm = convertKeysToCamelCase(
