...
function createHoleResultWithTimestamp(
  holeNumber: number,
  winner: HoleWinner,
  timestamp: string,
  matchId: string = 'test-match-1'
  ...
  return {
    id: `result-${holeNumber}-${timestamp}`,
    matchId,
    holeNumber,
    winner,
    timestamp,
...
describe('calculateMatchState', () => {
  describe('Basic Score Calculation', () => {
    ...
    it('should calculate Team A leading by 1', () => {
      const match = createMockMatch();
      const results = createHoleResults([{ hole: 1, winner: 'teamA' }]);
    ...
    it('should use latest result when duplicate hole numbers exist', () => {
      const match = createMockMatch();
      const early = createHoleResultWithTimestamp(1, 'teamA', '2024-01-01T08:00:00.000Z');
      const late = createHoleResultWithTimestamp(1, 'teamB', '2024-01-01T09:00:00.000Z');

      const state = calculateMatchState(match, [early, late]);

      expect(state.holesPlayed).toBe(1);
      expect(state.currentScore).toBe(-1);
    });

    it('should ignore invalid hole numbers', () => {
      const match = createMockMatch();
      const results = [
        createHoleResult(1, 'teamA'),
        createHoleResult(0, 'teamB'),
        createHoleResult(19, 'teamB'),
      ];
  ...
  describe('Closeout Detection', () => {
    ...
    it('should format final-hole win as 1 UP (not 1&0)', () => {
      const match = createMockMatch();
      const results = createHoleResults([
