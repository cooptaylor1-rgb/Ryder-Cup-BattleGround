...
describe('Match Play Handicap Allocation', () => {
  ...
  describe('getMatchPlayStrokesOnHole (differential-based)', () => {
    const standardHoleHandicaps = [7, 11, 3, 13, 9, 1, 15, 5, 17, 8, 16, 10, 4, 12, 6, 18, 2, 14];

    it('should only give strokes to higher handicap team', () => {
      // Team A: 10 handicap, Team B: 15 handicap
      // Differential = 5, Team B gets 5 strokes
      // Hole 1 has HCP 7, which is NOT in top 5, so 0 strokes
      const result = getMatchPlayStrokesOnHole(1, 10, 15, standardHoleHandicaps);
      expect(result.teamAStrokes).toBe(0);
      expect(result.teamBStrokes).toBe(0); // HCP 7 doesn't get stroke with 5 differential
    });

    it('should give stroke on hardest holes', () => {
      // Team A: 10 handicap, Team B: 15 handicap
      // Differential = 5, Team B gets 5 strokes on HCP 1-5 holes
      // Hole 6 has HCP 1 (hardest), so Team B gets 1 stroke
      const result = getMatchPlayStrokesOnHole(6, 10, 15, standardHoleHandicaps);
      expect(result.teamAStrokes).toBe(0);
      expect(result.teamBStrokes).toBe(1);
    });

    it('should return zero strokes for equal handicaps', () => {
      const result = getMatchPlayStrokesOnHole(1, 12, 12, standardHoleHandicaps);
      expect(result.teamAStrokes).toBe(0);
      expect(result.teamBStrokes).toBe(0);
    });

    it('should give strokes to Team A when Team A has higher handicap', () => {
      // Team A: 18 handicap, Team B: 10 handicap
      // Differential = 8, Team A gets 8 strokes
      // Hole 6 has HCP 1 (hardest), so Team A gets 1 stroke
      const result = getMatchPlayStrokesOnHole(6, 18, 10, standardHoleHandicaps);
      expect(result.teamAStrokes).toBe(1);
      expect(result.teamBStrokes).toBe(0);
    });

    it('should handle large differential (2 strokes on hardest holes)', () => {
      // Team A: 36 handicap, Team B: 10 handicap
      // Differential = 26, which means 2 strokes on all holes, some with more
      // Hole 6 has HCP 1, should get 2 strokes
      const result = getMatchPlayStrokesOnHole(6, 36, 10, standardHoleHandicaps);
      expect(result.teamAStrokes).toBe(2);
      expect(result.teamBStrokes).toBe(0);
    });

    it('should correctly apply to real match scenario (bug fix test)', () => {
      // Real bug scenario: Team Europe gross 5, gets 1 stroke (HCP 5), net 4
      // Team USA gross 4. Should be HALVED, not Team USA win.

      // Team USA: 10 handicap, Team Europe: 15 handicap
      // Differential = 5, so only Team Europe gets strokes
      // Hole 8 has HCP 5 in standard handicaps
      const result = getMatchPlayStrokesOnHole(8, 10, 15, standardHoleHandicaps);

      // Team USA should NOT get any strokes (lower handicap team)
      expect(result.teamAStrokes).toBe(0);
      // Team Europe should get 1 stroke (HCP 5 is within their 5 strokes)
      expect(result.teamBStrokes).toBe(1);

      // Now if Team USA scores 4 gross, Team Europe scores 5 gross:
      // Team USA net: 4 - 0 = 4
      // Team Europe net: 5 - 1 = 4
      // Result: HALVED (not Team USA wins)
      const teamUSAGross = 4;
      const teamEuropeGross = 5;
      const teamUSANet = teamUSAGross - result.teamAStrokes;
      const teamEuropeNet = teamEuropeGross - result.teamBStrokes;
      expect(teamUSANet).toBe(teamEuropeNet); // HALVED!
    });
  });
