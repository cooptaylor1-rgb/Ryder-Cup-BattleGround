      ...
      <div className="grid gap-[var(--space-5)] px-[var(--space-5)] py-[var(--space-5)] lg:grid-cols-[minmax(0,1.2fr)_18rem]">
        ...
        <div className="grid gap-[var(--space-3)] sm:grid-cols-3 lg:grid-cols-1">
          <FinanceFactCard
            ...
            detail="Confirmed money already on the books."
          />
          <FinanceFactCard
            label="Outstanding"
            value={formatCents(summary.outstandingBalance)}
            detail="Still sitting out in the group."
          />
          <FinanceFactCard
            label="Settled players"
            value={`${settledPlayers}/${owingPlayers || totalPlayers}`}
            detail="How many people are actually done."
...
export function FinanceTabButton({
  active,
  label,
  icon,
  onClick,
      ...
      <div className="flex flex-wrap items-end justify-between gap-[var(--space-4)]">
        <div>
          ...
          <p className="mt-[var(--space-2)] text-sm text-[var(--ink-secondary)]">
            collected of {formatCents(totalCollectable)}
          </p>
        </div>
...
export function ChargeComposer({
  ...
  const categories = Object.entries(DUES_CATEGORIES) as Array<
    [DuesCategory, { label: string; icon: string; color: string }]
  >;
      ...
      <div className="mt-[var(--space-5)] space-y-[var(--space-5)]">
        ...
        <FormField label="Amount per player">
          <div className="relative">
            <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-sm font-semibold text-[var(--ink-tertiary)]">
              $
            </span>
            <input
              type="text"
        ...
        {chargeTargetCount > 0 && chargeTotalCents > 0 ? (
          <div className="rounded-[1.2rem] border border-[var(--masters)]/18 bg-[color:var(--masters)]/8 px-[var(--space-4)] py-[var(--space-4)]">
            <p className="text-sm font-medium text-[var(--masters)]">
              {chargeTargetCount} charge{chargeTargetCount === 1 ? '' : 's'} for a total of{' '}
