    ...
    const handleJoin = async () => {
        ...
        try {
            ...
            if (result.success && result.synced > 0) {
                // Find the trip ID from the join result
                const trip = await syncService.fetchTripByShareCode(shareCode.trim());
                if (trip) {
...
export function ShareTripCard({ shareCode, className }: ShareTripProps) {
    ...
    return (
        <div className={cn('p-4 rounded-xl bg-masters-primary/5 border border-masters-primary/20', className)}>
            ...
            >
                <span className="text-2xl font-mono tracking-[0.5em] font-bold">
                    {shareCode}
                </span>
                <span className="text-sm text-masters-primary">
                    {copied ? 'Copied!' : 'Copy'}
                </span>
            </button>
        </div>
