...
export function OneHandedScoringPanel({
  ...
  const handleDrag = useCallback(
    (event: TouchEvent | MouseEvent | PointerEvent, info: PanInfo) => {
      if (disabled) return;
      const { offset } = info;
  ...
  const scoreColor =
    currentScore > 0 ? teamAColor : currentScore < 0 ? teamBColor : COLORS.textSecondary;
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ fontSize: '0.75rem', color: COLORS.textSecondary, marginBottom: '12px' }}>
          Match Score
        </p>
        <motion.p
          key={currentScore}
        ...
        <p style={{ fontSize: '0.75rem', color: COLORS.textTertiary, marginTop: '12px' }}>
          thru {holesPlayed}
        </p>
      ...
      >
        ...
        >
          <button
            ...
            aria-label={`Previous hole${holeNumber <= 1 ? ' (disabled)' : ''}`}
            style={{
              width: 48,
              height: 48,
              borderRadius: '12px',
              ...
              alignItems: 'center',
              justifyContent: 'center',
              cursor: holeNumber <= 1 ? 'not-allowed' : 'pointer',
              opacity: holeNumber <= 1 ? 0.3 : 1,
            }}
          >
          ...
          <button
            ...
            aria-label={`Next hole${holeNumber >= 18 ? ' (disabled)' : ''}`}
            style={{
              width: 48,
              height: 48,
              borderRadius: '12px',
              ...
              alignItems: 'center',
              justifyContent: 'center',
              cursor: holeNumber >= 18 ? 'not-allowed' : 'pointer',
              opacity: holeNumber >= 18 ? 0.3 : 1,
            }}
          >
