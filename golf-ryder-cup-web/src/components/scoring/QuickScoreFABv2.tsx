...
export function QuickScoreFABv2({
  ...
  const inProgressMatches = useLiveQuery(async () => {
    ...
    const sessions = await db.sessions.where('tripId').equals(currentTrip.id).toArray();
  ...
  useEffect(() => {
    async function updateActiveMatch() {
      ...
      const holeResults = await db.holeResults.where('matchId').equals(match.id).toArray();
  ...
  // Let the dedicated "my match" widget own the no-match affordance. Otherwise we can
  // end up with two floating buttons competing for the same corner of the screen.
  if (!activeMatchData) {
    return null;
  }
