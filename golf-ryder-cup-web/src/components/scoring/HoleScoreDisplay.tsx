...
export function HoleScoreDisplay({
    ...
    // Calculate strokes on this specific hole
    const teamAStrokesOnHole = useMemo(() => {
        return getStrokesOnHole(holeNumber, teamAHandicapStrokes, holeHandicaps);
    }, [holeNumber, teamAHandicapStrokes, holeHandicaps]);

    const teamBStrokesOnHole = useMemo(() => {
        return getStrokesOnHole(holeNumber, teamBHandicapStrokes, holeHandicaps);
    }, [holeNumber, teamBHandicapStrokes, holeHandicaps]);

    // Calculate net scores
    const teamANetScore = teamAGrossScore !== null
        ? teamAGrossScore - teamAStrokesOnHole
        : null;

    const teamBNetScore = teamBGrossScore !== null
        ? teamBGrossScore - teamBStrokesOnHole
        : null;
            ...
            {showStrokeDetails && (teamAStrokesOnHole > 0 || teamBStrokesOnHole > 0) && (
                ...
                >
                    <p className="text-center" style={{ color: 'var(--ink-secondary)' }}>
                        {teamAStrokesOnHole > 0 && teamBStrokesOnHole > 0 ? (
                            <>
                                Both teams receive strokes: {teamAName} gets {teamAStrokesOnHole}, {teamBName} gets {teamBStrokesOnHole}
                            </>
                        ) : teamAStrokesOnHole > 0 ? (
                            <>
                                {teamAName} receives {teamAStrokesOnHole} stroke{teamAStrokesOnHole > 1 ? 's' : ''} on this hole
                            </>
                        ) : (
                            <>
                                {teamBName} receives {teamBStrokesOnHole} stroke{teamBStrokesOnHole > 1 ? 's' : ''} on this hole
                            </>
                        )}
                    </p>
                </div>
