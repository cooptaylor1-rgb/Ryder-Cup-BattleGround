 ...
 *
 * Handles errors in social routes with context-specific messaging.
...
export default function SocialError({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    Sentry.captureException(error, { tags: { feature: 'social' } });
  }, [error]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[var(--canvas)]">
      ...
      <h1 className="text-2xl font-semibold mb-3 text-[var(--ink)]">Couldn&apos;t Load Social Feed</h1>

      <p className="text-center max-w-md mb-8 text-[var(--ink-secondary)]">
        There was a problem loading the social feed. Please try again.
      </p>
