...
const ROUTE_CARDS: StatsRouteCard[] = [
  {
    id: 'trip-stats',
    title: 'Trip stats',
...
export default function StatsPage() {
  ...
  const { currentTrip: activeTrip } = useTripStore();
        ...
        <section className="overflow-hidden rounded-[2rem] border border-[var(--masters)]/14 bg-[linear-gradient(135deg,rgba(10,80,48,0.97),rgba(4,52,30,0.98))] text-[var(--canvas)] shadow-[0_28px_64px_rgba(5,58,35,0.22)]">
          <div className="grid gap-[var(--space-5)] px-[var(--space-5)] py-[var(--space-5)] lg:grid-cols-[minmax(0,1.22fr)_18rem]">
            ...
            <div className="grid gap-[var(--space-3)] sm:grid-cols-3 lg:grid-cols-1">
              <StatsFactCard label="Trip" value={activeTrip.name} detail="The current competition context." valueClassName="font-sans text-[1rem] not-italic leading-[1.25]" />
              <StatsFactCard label="Stat rooms" value={ROUTE_CARDS.length} detail="The core places where story-level numbers live." />
              <StatsFactCard label="Best next stop" value="Trip stats" detail="The broadest look at how the weekend is actually unfolding." valueClassName="font-sans text-[1rem] not-italic leading-[1.25]" />
            </div>
          </div>
        </section>
