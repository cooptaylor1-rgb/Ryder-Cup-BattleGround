...
export default function CaptainError({ error, reset }: ErrorPageProps) {
  ...
  return (
    <div className="min-h-screen pb-nav page-premium-enter texture-grain flex flex-col items-center justify-center p-6 bg-[var(--canvas)]">
      <div className="w-20 h-20 rounded-2xl flex items-center justify-center mb-6 bg-[var(--error)]/10">
        <AlertTriangle size={40} className="text-[var(--error)]" />
      </div>

      <h1 className="text-2xl font-semibold mb-3 text-[var(--ink)]">Captain Dashboard Error</h1>

      <p className="text-center max-w-md mb-8 text-[var(--ink-secondary)]">
        Something went wrong loading the captain dashboard. Your trip data is safe.
      </p>
