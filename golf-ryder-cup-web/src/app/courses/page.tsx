...
export default function CourseLibraryPage() {
  ...
  const handleScorecardData = useCallback(
    async (data: {
      courseName?: string;
      teeName?: string;
      rating?: number;
      ...
      try {
        ...
        if (data.teeSets && data.teeSets.length > 0) {
          ...
          showToast(
            'success',
            `Course "${data.courseName || 'Scanned Course'}" created with ${teeSetData.length} tee sets!`
          );
        } else {
          ...
          showToast(
            'success',
            `Course "${data.courseName || 'Scanned Course'}" created from scorecard!`
          );
        }
  ...
  return (
    <div className="min-h-screen page-premium-enter texture-grain bg-[var(--canvas)]">
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-[var(--ink-secondary)] mt-1">
                Save courses to your library and quickly add them to any trip. Hole pars, handicaps,
                and tee sets are preserved.
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Secondary add paths. When the course directory API is
                    not ready (default), the Course Directory tile is a
        ...
        <div className={cn('grid gap-3', apiConfigured ? 'grid-cols-2' : 'grid-cols-1')}>
          {apiConfigured && (
            ...
            >
              ...
              <div className="font-medium text-[var(--ink-primary)]">Course Directory</div>
