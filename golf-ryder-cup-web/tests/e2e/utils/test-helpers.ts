...
export async function waitForNetworkIdle(
  page: Page,
  timeout = TEST_CONFIG.timeouts.networkIdle
): Promise<void> {
  try {
...
export async function expectPageReady(
  ...
  try {
    ...
    if (hasContentContainer) {
      ...
      // Fallback: direct navigation when bottom nav is hidden (e.g., desktop)
      ...
      await page.goto(pathMap[section]);
...
export async function seedUserProfile(
  ...
  await page.evaluate((profileData) => {
    const userProfile = {
      ...
      handicapIndex: profileData.handicap,
      hasCompletedOnboarding: profileData.hasCompletedOnboarding ?? true,
      isProfileComplete: profileData.isProfileComplete ?? true,
      createdAt: profileData.createdAt,
      updatedAt: profileData.updatedAt,
    };

    // Persisted auth store state (zustand persist)
    ...
    localStorage.setItem(
      'golf-app-users',
      JSON.stringify({
        [userProfile.id]: { profile: userProfile, pin: '0000' },
      })
    );
...
export async function verifyTapTargets(
  ...
  passed: boolean;
  failures: Array<{ element: string; width: number; height: number }>;
}> {
  const {
...
export async function verifyNavigationHistory(
  ...
  const forwardWorks =
    typeof expectedForwardUrl === 'string'
...
export async function navigateViaBottomNav(
  ...
  if (await navButton.isVisible({ timeout: TEST_CONFIG.timeouts.fast })) {
    ...
    try {
      const pathRegex = new RegExp(`${pathMap[section]}(\\?|$)`);
      await page.waitForURL(pathRegex, { timeout: TEST_CONFIG.timeouts.standard });
      await waitForStableDOM(page);
      return;
    } catch {
...
export async function enableCaptainMode(page: Page, pin?: string): Promise<boolean> {
  ...
  return await captainIndicator
    .isVisible({ timeout: TEST_CONFIG.timeouts.fast })
    .catch(() => false);
}
