...
- 16:15 — Shipped batch: Captain Messages, Draft, and Captain Settings now use the shared `PageHeader` and shared `BottomNav` (replacing bespoke header and hand-rolled nav markup) for consistent premium navigation across captain tools. (commit TBD)
